...
const statesByName: Record<string, IStateResult> = {};
const statesByAbbr: Record<string, IStateResult> = {};
for (const state of new UsaStates().states) {
  statesByName[state.name] = state;
  statesByAbbr[state.abbreviation] = state;
}

function inBayArea(latLong: LatLong) {
  return (
    latLong &&
    latLong.lat > 36.878 &&
    latLong.lat < 38.859 &&
...
function inSeattleArea(latLong: LatLong) {
  return (
    latLong &&
    latLong.lat > 46.724 &&
    latLong.lat < 48.309 &&
...
type SearchSource = {
  name?: string;
  field?: (resp: ResponsePublic) => string;
  filter?: (resp: ResponsePublic) => boolean;
  summerFilter?: (resp: ResponsePublic) => boolean;
  view?: SidebarView;
};
...
const SearchBar = (props: {
  ...
  const years = useSelector((state) =>
    Object.keys(state.responses ?? {}).map((year) => parseInt(year, 10)),
  );

  const classYear = useSelector((state) => state.classYear);
  ...
  React.useEffect(() => {
    ...
    $(inputRef.current).on("keydown", (event) => {
      if (event.key === "Escape") {
    ...
    <>
      <div
        style={{
          ...
          maxWidth: `calc(100% - ${searchBarPadding * 3 + searchBarHeight}px)`,
          touchAction: "none",
        }}
      >
        <div
        ...
        <button
          ...
          onClick={() => {
            ...
            let zoom = mapRef.current.getZoom() + 1;
            console.log(zoom);
            // Correction for sidebar.
            if (showingSidebar && zoom <= 20) {
              if (sidebarVertical) {
                lng -=
        ...
        <div className="modal-dialog">
          <div className="modal-content">
            ...
            <div className="modal-body">
              ...
              <ul>
                <li>
                  You can search for people, companies, majors, or regions ("Bay
                  Area"). Anything that was submitted on the form!
                </li>
                <li>
                ...
                <li>
                  ...
                  publicize it beyond the Class of 2020, as per the Honor Code.
                </li>
              </ul>
            </div>
          </div>
